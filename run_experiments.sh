...
reset_neo4j() {
    ...
    cat neo4j_schema.cypher | docker exec -i flights_neo4j cypher-shell -u neo4j -p password123 >/dev/null
}
...
reset_cassandra() {
    ...
    cat cassandra_schema.cql | docker exec -i flights_cassandra cqlsh >/dev/null
}
...
for ds in 25 50 75 100; do
    ...
    reset_neo4j
    reset_cassandra
    python insert_data.py --dataset $ds
    python benchmark.py --dataset $ds --append
done
